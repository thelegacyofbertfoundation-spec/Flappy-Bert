// Server-side badge gating — mirror of the badge logic wired into bot.js.
...
function allowedBadges(submitted, validatedScore, existing) {
  ...
  for (const b of Array.isArray(submitted) ? submitted : []) {
    ...
    if (b in BADGE_MIN_SCORE && Number(validatedScore) < BADGE_MIN_SCORE[b]) continue;
    accepted.add(b);
  }
  return [...accepted].slice(0, VALID_BADGES.length);
}

module.exports = { allowedBadges, VALID_BADGES, BADGE_MIN_SCORE };
