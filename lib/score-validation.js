...
function scoreVerdict(input, L = LIMITS) {
  ...
  // Level is client-reported and decoupled from score; clamp it to what the
  // score could legitimately produce (~1 level per 10 pipes/score, +1 base) so a
  // fabricated level (e.g. 1000 at score 1) can't render a fake MAX LEVEL on the
  // public cards. Score grows at least as fast as pipes (combo/near-miss bonuses
  // inflate score, not level), so a legit run is never clamped.
  const cappedLevel = Math.min(lvl, Math.floor(n / 10) + 1);

  return { valid: true, level: cappedLevel, coins };
}
