...
## Tests
`npm test` runs `node --test tests/*.test.js`. Test mirror files in `tests/lib/` are pure-JS replicas of in-HTML logic (drift risk, accepted per the 2026-04-30 spec). Current count: 80 (18 tournaments-config + 14 score-validation + 7 badge-allowlist + 6 sanitize-name + 6 csv-cell + 5 fx-frenzy-timer + 5 ghost-challenge + 4 season-label + 4 homing-enemy + 4 fx-spawn-cap + 4 fx-magnet-timer + 3 fx-game-over-sequence). The score-validation + badge-allowlist suites include the 2026-06-19 security regressions (level→score clamp, ungated-badge min-score gate); ghost-challenge covers the Beat-My-Ghost param helpers (`lib/ghost-challenge.js`).
