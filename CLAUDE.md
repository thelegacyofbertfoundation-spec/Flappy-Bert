...
- `/resettournament <tournament_id> CONFIRM` — Admin: hard-DELETE a tournament's scores. Guarded since 2026-07-09 (`lib/reset-tournament-guard.js`): requires an exact known id + the literal uppercase `CONFIRM`; anything else deletes nothing and replies with usage + each tournament's current score-row count. (Previously auto-targeted the live tournament with no confirmation — a footgun during a live prize race.) This is DESTRUCTIVE and ignores the non-destructive `scoreResetAt` boundary; prefer `scoreResetAt` for score resets.
...
- **Score reset + prizes (data-driven, 2026-06-19):** a tournament config entry may carry two OPTIONAL fields — `scoreResetAt` (ISO-8601 UTC) and `prizes` (array of USD ints for places 1..N). Summer Session has `scoreResetAt: 2026-06-22T00:00:00Z` + `prizes: [100,60,40,30,20]`. The reset is **non-destructive + deterministic** (no cron, no wipe, no flag): `lib/tournament-reset.js` `effectiveResetSince(scoreResetAt, now)` returns the SQLite boundary only once `now ≥ scoreResetAt`, and `db.getTournamentLeaderboard`/`getTournamentPlayerRank` take an optional `since` → `played_at >= since`. Applied in all three reads (`GET /api/tournament/:id`, the bot `/tournament` card, the post-submit rank), each via `bot.js` `tournamentSince(id)`. Old scores stay in the DB (dormant after the boundary); reversible by removing `scoreResetAt`. The **prize ladder** is always-shown (positions 1..N render even when empty, "— up for grabs —"): `renderEntriesInto` (overlay) + `renderTournamentCard` (bot card, `options.prizes`). Prizes are **display-only** — they never touch score validation/coins/badges/rank. A tournament without these fields renders/behaves exactly as before. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-19-tournament-reset-prizes*`.
...
- **Production note (RESOLVED 2026-06-19):** the prod DB carried a duplicate April row (`april-flapoff-2026`) from a prior deploy alongside the canonical `april-fools-flapoff-2026`, so PAST TOURNAMENTS showed two April entries. Now auto-removed on startup: `bot.js` calls `db.deleteTournament('april-flapoff-2026')` after seeding — idempotent + self-healing (no-op once gone, re-removes if a bad seed recreates it). NOTE: `foreign_keys` is ON with no `ON DELETE CASCADE`, so `db.deleteTournament` clears the child `tournament_scores` FIRST then the row, atomically — a bare `DELETE FROM tournaments WHERE id=…` would throw a FK error if the orphan had any scores. Mechanism proven by `tools/verify-tournament-cleanup.cjs`.

## Roadmap
Per-feature implementation history, shipped-feature changelog entries, and next-session candidates have been moved to `docs/RESEARCH_LOG.md` (newest at bottom) for context hygiene. **Open improvements are consolidated in `docs/IMPROVEMENTS.md`** (from the 2026-07-09 critique: the Sep-1-gated balance changes, high-value ops/test work, UX polish, hygiene, and post-deploy watches) — read it before planning new work.

## Game mechanics
- **Fixed 60Hz timestep (2026-07-09):** `gameLoop(ts)` drives `update()` from a fixed-timestep accumulator (`LOOP_STEP_MS = 1000/60`, catch-up capped at `LOOP_MAX_STEPS = 4`, 1ms epsilon so 60Hz jitter never double-steps) — previously one update ran per rAF frame, so 120Hz devices played a 2x-speed game and frame-throttling was a score exploit. At a steady 60Hz behavior is identical to before (exactly 1 step/frame). `resetGameLoopClock()` clears the accumulator on resume-from-pause / tab-visible so no catch-up burst kills the player. Loop state lives OUTSIDE `G` (anti-tamper setters never see it). Mirror logic: `tests/lib/fixed-timestep.js`.
- **Auto-pause on backgrounding (2026-07-09):** `visibilitychange`(hidden) + `pagehide` route through `togglePause()` while `state==='playing'` (overlay + ⏸/▶ glyph stay in sync; the landscape auto-pause now also routes through `togglePause()`, fixing its old glyph desync). Never auto-resumes.
- **Levels:** Every 10 pipes cleared = +1 level. Speed, gap, pipe interval scale with level; plateaus are staggered since the 2026-07-10 ramp change — speed maxes at level 12 (score 110, top 5.5 @ 0.25/level), pipe interval at level 15, gap at level 20 (110px). Level transitions fire a sweep banner + fanfare.
...
- Markup is grouped into named `fx-go-step` sections (`title`, `hero`, `stats`, `rewards`, `actions`) revealed via a `setTimeout` chain over ~2.9s. Since 2026-07-09 the steps are RESET (`fx-go-show` removed) at sequence start — previously the class was never removed, so the reveal only sequenced on the first death of a session — and a dim `#goSkipHint` "TAP TO SKIP ▸" shows while the reveal runs (hidden on completion/skip/cancel).
...
## Mid-tournament fix policy (Summer Session, until 2026-09-01)
2026-07-09 hardening pass shipped ONLY changes that don't make post-change runs harder than the conditions the current leader's score was set under (top score was 240/500 at the time): leaderboard tiebreak (set BEFORE ties exist), fixed 60Hz timestep (60Hz players bit-identical; 120Hz players get the intended = easier game; frame-throttle exploit closed), crash safety, `/resettournament` guard, auto-pause, and UX/copy fixes.
...
**DEFERRED to the Sep 1 season boundary — do NOT ship mid-tournament:**
- Remove/neuter the shop 2X multiplier's effect on tournament scoring (halves the skill needed to reach the 500 cap; removing it mid-race would lock in any multiplier-assisted incumbent score).
- Extend the difficulty curve past level 20 (everything is flat by score ~190 — speed even earlier, at 110, since the 2026-07-10 ramp change — while the cap is 500; raising the top end mid-race disadvantages challengers chasing an existing plateau-era score).
- Coin-economy rebalance (quadratic combo faucet, double-count bug where live-credited near-miss/combo/magnet/shield coins are banked AGAIN via `G.coins += G.coinsEarned` at game over, ~915-coin one-time sink) — coins buy the 2x multiplier, so nerfing the faucet mid-season slows new challengers' access to it.
- Tighten the server 5/sec score-rate cap (only safe once fixed-timestep clients have fully rolled out; no cached 120Hz client can then legitimately score ~10/sec).
...
- **Leaderboard tiebreak (2026-07-09)** — all 5 board/rank queries (`getWeeklyLeaderboard`, `getPlayerRank`, `getTournamentLeaderboard`, `getTournamentPlayerRank`, `archiveWeek`) order by `best_score DESC, best_at ASC` where `best_at` = earliest `played_at` at which the player reached their best score (within the same week/tournament/`since` window — a pre-`scoreResetAt` achievement can't win a post-boundary tie). The `RANK()` windows use the identical composite ordering so the caption rank always matches card row order. Shipped before anyone reached the 500 cap so no existing standings changed. Score is capped at 500 and ties WILL happen at the cap — without this, USD prize order among tied players was undefined SQLite row order.
- **Crash safety (2026-07-09)** — `process.on('unhandledRejection')` logs + keeps serving (a failed Telegram send must not kill the score API; node:20 default would exit); `uncaughtException` logs + exits 1 (Render restarts clean). Every fire-and-forget/unprotected `send*` in bot.js is wrapped in `safeSend` (`lib/safe-send.js` — logs, never throws). `bot.on('polling_error')` logs throttled to one line per error code per 30s (409 deploy-overlap storms don't spam).
...
## Tests
`npm test` runs `node --test tests/*.test.js`. Test mirror files in `tests/lib/` are pure-JS replicas of in-HTML logic (drift risk, accepted per the 2026-04-30 spec). Current count: 127 (21 tournaments-config + 14 score-validation + 7 badge-allowlist + 6 sanitize-name + 6 csv-cell + 5 fx-frenzy-timer + 5 ghost-challenge + 4 tournament-reset + 4 season-label + 4 homing-enemy + 4 fx-spawn-cap + 4 fx-magnet-timer + 3 fx-game-over-sequence + 8 fixed-timestep + 7 rank-nudge + 6 safe-send + 8 reset-tournament-guard + 3 leaderboard-tiebreak + 8 difficulty-curve — the tiebreak suite drives the REAL db.js queries against an isolated temp DB via the `FLAPPY_DATA_DIR` env override in db.js; tests must NEVER touch the live `./flappy_bert.db`). The score-validation + badge-allowlist suites include the 2026-06-19 security regressions (level→score clamp, ungated-badge min-score gate); ghost-challenge covers the Beat-My-Ghost param helpers (`lib/ghost-challenge.js`); tournament-reset covers the time-boundary reset helpers (`lib/tournament-reset.js`).
